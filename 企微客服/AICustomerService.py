...
# 配置信息 - 请根据您的实际情况在config.ini中修改
cfg = ConfigParser()
cfg.read('./config.ini')
CONFIG = dict(cfg.items('CONFIG'))
MEMCACHE_CONFIG = dict(cfg.items('MEMCACHE'))

# 初始化 Memcache 客户端
try:
    mc = base.Client((MEMCACHE_CONFIG['host'],int(MEMCACHE_CONFIG['port'])),
                     connect_timeout=int(MEMCACHE_CONFIG['connect_timeout']),
                     timeout=int(MEMCACHE_CONFIG['timeout']))
    # 测试连接
...
CACHE_KEYS = {
    ...
    'CONVERSATION_PREFIX': 'dify_conversation_'
}
...
def process_single_kf_message(msg, open_kf_id):
    ...
    try:
        ...
        if msgtype == 'text':
            ...
            print(f"💬 用户文本消息: {content}")

            # 检查是否是重置对话的指令
            if content.strip().lower() in ['重置', 'reset', '重新开始', '清空对话']:
                reset_result = reset_user_conversation(external_userid)
                if reset_result:
                    reply_content = "对话已重置，我们重新开始吧！"
                else:
                    reply_content = "对话重置失败，请稍后再试。"
                send_kf_message(open_kf_id, external_userid, reply_content)
            else:
                # 调用Dify API获取回复
                dify_reply = call_dify_api(content, external_userid)

                # 发送回复消息
                if dify_reply and dify_reply != 'error':
                    send_kf_message(open_kf_id, external_userid, dify_reply)
                else:
                    # Dify调用失败时的备用回复
                    error_reply = "抱歉，我现在无法回答您的问题，请稍后再试。"
                    send_kf_message(open_kf_id, external_userid, error_reply)
            ...
            # 可以在这里添加图片处理逻辑
            reply_content = "我收到了您的图片，但目前还不支持图片分析功能。请用文字描述您的问题。"
            send_kf_message(open_kf_id, external_userid, reply_content)
            ...
            # 可以在这里添加语音处理逻辑
            reply_content = "我收到了您的语音消息，但目前还不支持语音识别功能。请用文字描述您的问题。"
            send_kf_message(open_kf_id, external_userid, reply_content)
            ...
            # 可以在这里添加文件处理逻辑
            reply_content = "我收到了您的文件，但目前还不支持文件分析功能。请用文字描述您的问题。"
            send_kf_message(open_kf_id, external_userid, reply_content)

        else:
            print(f"❓ 其他类型消息:")
            print(f"   完整内容: {json.dumps(msg, indent=2, ensure_ascii=False)}")
            reply_content = "抱歉，我暂时无法处理这种类型的消息。请发送文字消息。"
            send_kf_message(open_kf_id, external_userid, reply_content)
...
def get_conversation_id(user_id):
    ...
    conversation_key = f"{CACHE_KEYS['CONVERSATION_PREFIX']}{user_id}"
    return cache_get(conversation_key)

def set_conversation_id(user_id, conversation_id):
    """设置用户的conversation_id"""
    conversation_key = f"{CACHE_KEYS['CONVERSATION_PREFIX']}{user_id}"
    return cache_set(conversation_key, conversation_id, expire=86400 * 30)  # 30天过期

def reset_user_conversation(user_id):
    """重置用户的对话上下文"""
    conversation_key = f"{CACHE_KEYS['CONVERSATION_PREFIX']}{user_id}"
    result = cache_delete(conversation_key)
    if result:
        print(f"🔄 已重置用户 {user_id} 的对话上下文")
    return result

def call_dify_api(message, user_id):
    """调用Dify API获取回复，自动管理conversation_id"""
    try:
        # 先尝试获取用户的conversation_id
        ...
        # 如果有conversation_id则加入请求
        if conversation_id:
            data["conversation_id"] = conversation_id
            print(f"🔗 使用已有的conversation_id: {conversation_id}")
        else:
            print(f"🆕 首次对话，将创建新的conversation_id")

        response = requests.post(url, json=data, headers=headers, timeout=30)

        if response.status_code == 200:
            result = response.json()
            answer = result.get('answer', '抱歉，我无法理解您的问题。')

            # 获取并保存conversation_id
            new_conversation_id = result.get('conversation_id')
            if new_conversation_id:
                if not conversation_id:
                    # 首次对话，保存新的conversation_id
                    set_conversation_id(user_id, new_conversation_id)
                    print(f"💾 保存新的conversation_id: {new_conversation_id}")
                elif new_conversation_id != conversation_id:
                    # conversation_id发生变化，更新缓存
                    set_conversation_id(user_id, new_conversation_id)
                    print(f"🔄 更新conversation_id: {conversation_id} -> {new_conversation_id}")

            print(f"✅ Dify API调用成功，回复: {answer[:50]}...")
            return answer
        else:
            print(f"❌ Dify API调用失败: 状态码 {response.status_code} 内容: {response.text}")
            return '抱歉，服务暂时不可用，请稍后再试。'

    except Exception as e:
        print(f"❌ 调用Dify API失败: {e}")
        import traceback
        traceback.print_exc()
...
def reset_conversation_api():
    ...
    try:
        ...
        result = reset_user_conversation(user_id)

        if result:
            return {"status": "success", "message": f"用户 {user_id} 的对话已重置"}
        else:
            return {"status": "failed", "message": "重置失败，用户可能没有对话记录"}, 404

    except Exception as e:
        return {"error": f"Reset failed: {e}"}, 500
...
def get_conversation_api():
    ...
    try:
        ...
        if conversation_id:
            ...
            return {"user_id": user_id, "conversation_id": None, "message": "用户还没有对话记录"}
    except Exception as e:
        return {"error": f"Get conversation failed: {e}"}, 500

if __name__ == '__main__':
    print("🚀 服务启动中...")
    print("📋 功能说明:")
    print("   - 企业微信Webhook接收: /webhook")
    print("   - 服务状态查询: /status")
    print("   - 清空缓存数据: /clear")
    print("   - 重置用户对话: /reset_conversation")
    print("💡 用户可发送 '重置'、'reset'、'重新开始'、'清空对话' 来重置对话上下文")
    app.run(host='0.0.0.0', port=11850)
