...
def decrypt_msg(encrypt_msg, encoding_aes_key, corp_id):
    ...
    try:
        ...
        logger.debug(f"去填充后数据长度: {len(decrypted)}")
        ...
        # 提取消息内容 - 修复这里的逻辑
        if len(decrypted) < 20:  # 至少需要16字节随机数 + 4字节长度
            logger.error(f"解密数据过短: {len(decrypted)}")
            return None
        ...
        # # 读取4字节的消息长度（小端序）
        # if len(content) < 4:
        #     logger.error("内容长度不足4字节")
        #     return None
        #
        # xml_len = struct.unpack("<I", content[:4])[0]  # 使用小端序
        # logger.debug(f"XML内容长度: {xml_len}")
        ...
        # 先尝试大端序
        xml_len = struct.unpack(">I", content[:4])[0]
        logger.debug(f"XML内容长度（大端序）: {xml_len}")

        # 如果大端序结果不合理，尝试小端序
        if xml_len > len(content) - 4 or xml_len <= 0:
            xml_len = struct.unpack("<I", content[:4])[0]
            logger.debug(f"XML内容长度（小端序）: {xml_len}")

        # 检查长度是否合理
        if xml_len > len(content) - 4 or xml_len <= 0:
            logger.error(f"XML长度异常: {xml_len}, 可用内容长度: {len(content) - 4}")
            return None
