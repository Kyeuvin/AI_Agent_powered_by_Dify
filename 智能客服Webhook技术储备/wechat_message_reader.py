...
def verify_url():
    ...
    try:
        ...
        print(f"🔐 验证参数:")
        print(f"   signature: {msg_signature}")
...
def process_decrypted_message(decrypted_msg):
    ...
    try:
        ...
        print_separator(f"消息解析 - 类型: {msg_type}")
...
def fetch_and_process_kf_messages(token, open_kf_id):
    """获取并处理企业微信客服消息"""
    try:
        print_separator(f"获取客服消息 - {open_kf_id}")
        ...
        if not cursor:
            ...
            print("🆕 首次获取消息，利用has_more参数获取最新消息")
            data = {
                "token": token,
                "limit": 1,  # 首次只获取1条消息
                "voice_format": 0,
        ...
        if result.get('errcode') == 0:
            ...
            print(f"   下一个Cursor: {next_cursor}")
            print(f"   是否还有更多: {'是' if has_more else '否'}")
            ...
            if not cursor and msg_list:
                ...
                if has_more == 1:# 如果还有更多消息，说明我们获取的是历史消息，需要跳到最新
                    ...
                    print("🚀 采用跳跃策略获取最新消息...")# 策略：不断获取消息直到has_more为0，然后取最后一批的最新消息
                    ...
                    if latest_messages:
                        ...
                        print(f"✅ 成功获取最新消息，时间: {format_timestamp(latest_messages[-1].get('send_time', 0))}")
                    else:
                        messages_to_process = []
                else:
                ...
                if next_cursor:
                    ...
                    print(f"🆕 设置初始Cursor为当前时间: {initial_cursor}")
            ...
            if messages_to_process:
                ...
                for i, msg in enumerate(messages_to_process, 1):
                    print_separator(f"消息 {i}/{len(messages_to_process)}")
...
def process_single_kf_message(msg, open_kf_id):
    ...
    try:
        ...
        print(f"   时间戳: {send_time}")
        print(f"   发送方: {'用户' if origin == 3 else '系统推送' if origin == 4 else f'未知({origin})'}")
        print(f"   外部用户ID: {external_userid}")
        print(f"   客服ID: {open_kf_id}")

        # 构造唯一消息ID用于去重
        unique_msg_id = f"kf_{msgid}_{external_userid}_{send_time}"

        print(f"🔑 唯一消息ID: {unique_msg_id}")
        ...
        print(f"📍 当前Cursor状态: {current_cursor}")

        print(f"📊 处理统计:")
        print(f"   已处理消息总数: {len(processed_messages)}")
        print(f"   活跃客服数量: {len(message_cursors)}")
...
def await_get_latest_messages(token, open_kf_id, access_token):
    ...
    try:
        ...
        if all_messages:
            ...
            print(f"   📝 最新Cursor: {current_cursor}")
